...
from tree.models import Tree, SubTree
from tree.serializers import TreeSerializer, SubTreeSerializer
...
def node_list(request):
    if request.method == 'GET':
        ...
        state = request.data.get('status', 'active')
        if valor:
...
def node_detail(request, pk):
    ...
    if request.method == 'GET':
        ...
        # Update the 'valor' field of the node
        valor = request.data.get('valor')
        if valor:
            node.valor = valor
            node.save()
...
@api_view(['PATCH'])
def node_state(request, pk):
    # Update the state of a Tree record and related SubTree records
